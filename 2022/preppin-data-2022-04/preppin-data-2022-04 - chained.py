...
df = df.merge(df_students, on='Student ID', how='inner')\
       .melt(id_vars='Student ID', var_name='Weekday', value_name='Method_of_Travel')\
       .assign(Method_of_Travel = lambda df_x: df_x['Method_of_Travel'].replace(travel_method_renames),
               Sustainable = lambda df_x: np.where(df_x['Method_of_Travel'].isin(sustainable_methods),
                              'Sustainable', 'Non-Sustainable'))\
       .groupby(['Sustainable', 'Method_of_Travel', 'Weekday'], as_index=False)\
...
df.rename(columns=lambda c: c.replace('_', ' ').replace('Pct', '%'))\
  .rename(columns={'Sustainable' : 'Sustainable?'})\
  .to_csv(r'.\outputs\output-2022-04.csv', index=False)
